...
def chama_segunda_tela():  # função responsavel por chamar a tela principal
    ...
    telaDeLogin.label_4.setText(
        "")  # Sempre limpo o campo de avisos para que quando o usuario corrigir o erro o campo esteja limpo
    ...
    try:
        ...
        cursor = conexao.cursor()
        cursor.execute(
            f"INSERT INTO log_usuario(descricao, idusuarios, dt_logusuario) VALUES ('NULL', {nomedousuario}, '{datahoje}');")
        conexao.commit()
...
def cadcliente():
    try:
        ...
        verificaCpfExiste = pd.read_sql(f"""select cpf_cnpj from parceiros p where cpf_cnpj ='{cpfdocliente}';""",
                                        conexao)
        if verificaCpfExiste.empty == True:
            ...
            pegaridbairro = pd.read_sql(
                f"select idbairro from bairros where nome='{bairrodocliente}' and cidades_idcidade='{cidadedocliente}'",
                conexao)
            if pegaridbairro.empty == True:

                cursor.execute(
                    f"""INSERT INTO bairros (cidades_estados_idestado, cidades_idcidade, nome) VALUES({estadodocliente}, {cidadedocliente}, '{bairrodocliente}');""")
                conexao.commit()
                return
            else:
            ...
            if not sitedocliente:
                QMessageBox.warning(TelaPrincipal, "Atenção",
                                    "Caso o cliente não possua um site\n preencha o campo com NÃO INFORMADO")
                return

            if not emaildocliente:
                QMessageBox.warning(TelaPrincipal, "Atenção",
                                    "Caso o cliente não possua um email\n preencha o campo com NÃO INFORMADO")
                return
            ...
            if verificaCpfExiste == cpfdocliente:
                QMessageBox.information(TelaPrincipal, "Aviso",
                                        "Cliente ja cadastrado anteriormente \nCPF {}".format(cpfdocliente))
            return
    except Exception as erro:
...
def geraRelatorioVendasEntSaida():
    try:
        ...
        df = df.rename(columns={'idvenda': 'Codigo Da Venda', 'tipo_negociacao_idtipo_negociacao': 'Tipo De Negociação',
                                'vendedores_usuarios_idusuarios': 'Usuario', 'vendedores_idvendedor': 'Vendedor',
                                "(DATE_FORMAT(data_venda , '%d/%m/%Y'))": 'Data Da Venda', 'vlr_total': 'Valor Total',
                                'nomecliente': 'Nome Do Cliente', 'nomeproduto': 'Nome Do Produto',
                                'quantproduto': 'Quantidade Vendida', 'precoproduto': 'Preço Do Produto',
                                'descproduto': 'Desconto', 'id_tipopagamento': 'Tipo De Pagamento',
                                'vezesdeparcelas': 'Vezes De Parcelas', 'observacao': 'Observação',
                                'dat_venv_fatuura': 'Data De Cobrança', 'entrada_saida': 'Entrda ou Saida'})
...
def gerarrelatorioprodutos():
    try:
        ...
        df = pd.read_sql("""SELECT idproduto, descricao, (DATE_FORMAT(dt_entrada , '%d/%m/%Y')), preco, observacao, marca,
            referencia from produtos where dt_entrada >='{}' and dt_entrada <='{}' order by idproduto DESC limit 1200""".format(
            datavenda1, datavenda2), conexao)

        # Renomeando as colunas
        # df.rename(columns={'$a':'a', '$b':'b', '$c':'c', '$d':'d', '$e':'e'})
        df = df.rename(columns={'idproduto': 'Codigo Do Produto', 'descricao': 'Descrição Do Produto',
                                "(DATE_FORMAT(dt_entrada , '%d/%m/%Y'))": 'Data Do Cadastro', 'preco': 'Preço',
                                'observacao': 'Obsercação Do Produto', 'marca': 'Fabricante'})
...
def vendasAvista():
    ...
    try:
        ...
        cursor.execute(""" select idvenda, nomecliente,
                ...
                and data_venda >= '{}' and data_venda <= '{}' order by idvenda DESC limit 1000000;""".format(datavenda1,
                                                                                                             datavenda2))
...
def realizarvendas():
    telaDeVendas.show()
    try:
        pegaridusuario = pd.read_sql(
            "select idusuarios from log_usuario where idusuarios is not null order by id_logusuario DESC limit 1",
            conexao)
...
def cadastrar_produtos():
    try:
        ...
        if pegardescicao.empty == True:
            ...
            pegarcategoria = pd.read_sql(f"select idcategoria from categorias c where descricao ='{categoria}'",
                                         conexao)
            ...
            VALUES  ('{}', '{}', {}, '{}','{}', '{}', '{}')""".format(categoria, descricao, preco, observacao, marca,
                                                                      ref, datadaentrada)
            if not preco:
            ...
            cursor.execute(
                "INSERT INTO estoque (estoque, produtos_idproduto) values ({},{})".format(estoque, tratadoproduto))
            conexao.commit()
            ...
            QMessageBox.warning(aviso, 'Aviso',
                                'Produto ja cadastrado anteriormente.   \n*{}*\n Caso não seja o mesmo produto adicione uma descrição diferente'.format(
                                    descricao))
...
def vendas_parceladas():
    ...
    try:
        ...
        cursor.execute("""select idvenda, nomecliente,
                ...
                where vezesdeparcelas > 1 and data_venda >= '{}' and data_venda <= '{}' order by idvenda DESC limit 1000000;""".format(
            datavenda1, datavenda2))
...
def deletarProduto():
    try:
        ...
        if not codigoestoque:
            QMessageBox.information(aviso, 'Aviso',
                                    'Para atualizar o estoque de um produto é necessario\n passar o codigo do PRODUTO e o codigo do ESTOQUE referente\n ao produto')
            return
...
def recuperausuario():
    ...
    try:
        retorno = pd.read_sql(
            f"select idusuarios from usuarios where nome ='{nomeUsuario}' and email ='{emailUsuario}';", conexao)
        if retorno.empty == True:
            QMessageBox.information(aviso, 'Atenção',
                                    'Usuario não encontrado, verifique se o nome e email estão corretos.\nCaso você não possua um usuario pode criar um na aba de cadastro.')
            return
        else:
        ...
        if novaSenha == verificaMesmaSenha:
            ...
            print(hashed)
            cursor.execute(
                f"update usuarios set senha ='{hashed}', updated_at ='{dataDaAtualizacao}' where idusuarios = {idUsuario};")
            conexao.commit()
...
def cadastrar_empresa():
    try:
        ...
        if VerificaEmpresa.empty == True:
            ...
            idbairro = pd.read_sql(
                f"select idbairro from bairros where nome='{nomedobairro}' and cidades_idcidade='{nomedacidade}'",
                conexao)
            if idbairro.empty == True:
                cursor = conexao.cursor()
                cursor.execute(
                    """INSERT INTO think.bairros (cidades_estados_idestado, cidades_idcidade, nome) VALUES ({}, {}, '{}');""".format(
                        idestado, idcidade, idbairros))
                conexao.commit()
                cursor.close()
            else:
            ...
            cursor.execute("""INSERT INTO think.empresa (cnpj, razao_social, nome_fantazia, tipo_empresa, atividade_principal, natureza_juridica, atividade_secundaria, situacao, capital_social, cep, complemento, email_empresa, telefone, abertura_empresa, porte_empresa, idbairro, idcidade, idestado)
                            VALUES('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', {}, {}, {})""".format(
                cnpj, razao_social, nome_fantazia, tipo_empresa, atividade_principal, natureza_juridica,
                atividade_secundaria, situacao, capital_socia, cep_empresa, complemento, email_empresa,
                telefone_empresa, abertura_empresa, porte_empresa, idbairro, idcidade, idestado))
            conexao.commit()
            cursor.close()
...
def consultas():
    ...
    descreicaoDaCategotia = pd.read_sql('select tipo_cliente from cat_cliente',
                                        conexao)  # Adicionando as categorias dos clientes
...
try:
    ...
    with open('logs\\Sistema_De_Vendas_Conexao_erro.txt', 'w') as arquivo:
        arquivo.write('Sistema_De_Vendas: "Erro: \n"{}\n'.format(
            erro))  # Abrindo o arquivo que contem a string de conexao com o DB
